  ...
  return (
    <div className="rounded-xl border border-nx-border bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        {campaignId != null ? (
          <span className="text-[11px] text-nx-muted">Solo esta campaña</span>
        ) : (
          <span className="text-[11px] text-nx-muted">Toda la empresa</span>
        )}
      </div>
      {error ? (
      ...
      {loading ? (
        <p className="mt-3 text-xs text-nx-muted">Cargando tareas…</p>
      ) : tasks.length === 0 ? (
        ...
        <div className="mt-3 space-y-4">
          {[...grouped.entries()].map(([kindLabel, rows]) => (
            <div key={kindLabel}>
              ...
              <ul className="mt-1.5 space-y-2 text-sm text-nx-ink">
                {rows.slice(0, 8).map((task) => (
                  <li
                    key={task.id}
                    className="flex flex-wrap items-start justify-between gap-2 rounded-lg border border-nx-border bg-nx-card-muted/80 px-2 py-1.5"
                  >
                    <div className="min-w-0 max-w-[42rem]">
                      ...
                      <p className="mt-1 text-xs text-nx-muted">
                        <span className="font-medium text-nx-ink">Campaña:</span>{' '}
                        {(task.campaign_name || '').trim() || '—'}
                        {' · '}
                        <span className="font-medium text-nx-ink">Prospecto:</span>{' '}
                      ...
                      {(task.reason || '').trim() ? (
                        <p className="mt-1 text-xs text-nx-muted">
                          <span className="font-medium text-nx-ink">Motivo:</span> {task.reason}
                        </p>
                      ) : null}
                      {(task.suggested_action || '').trim() ? (
                        <p className="mt-1 text-xs text-nx-muted">
                          <span className="font-medium text-nx-ink">Acción sugerida:</span>{' '}
                    ...
                    <Link
                      ...
                      className="shrink-0 text-xs font-semibold text-zinc-700 hover:text-zinc-900"
                    >
                      Abrir campaña
                    </Link>
                  </li>
              ...
              {rows.length > 8 ? (
                <p className="mt-1 text-[11px] text-nx-subtle">+{rows.length - 8} más…</p>
              ) : null}
            </div>
          ))}
        </div>
