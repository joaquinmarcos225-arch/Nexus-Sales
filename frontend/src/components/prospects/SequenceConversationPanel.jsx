...
const RESPONSE_CLASS_STYLES = {
  ...
  pedir_mas_info: 'bg-zinc-50 text-zinc-800 ring-zinc-600/20',
  derivar_a_otra_persona: 'bg-zinc-50 text-zinc-900 ring-zinc-600/20',
  contactar_mas_adelante: 'bg-zinc-50 text-zinc-800 ring-zinc-600/20',
  respuesta_automatica: 'bg-nx-card-muted text-nx-ink ring-nx-muted/20',
  desconocido: 'bg-nx-card-muted text-nx-muted ring-nx-muted/20',
}
  ...
  return (
    <div
      className={`rounded-lg border p-3 text-sm ${
        confirmed ? 'border-red-200 bg-red-50/60' : 'border-zinc-200 bg-zinc-50/60'
      }`}
    >
      ...
      <dl className="mt-2 space-y-1 text-xs text-nx-ink">
        {booking.meeting_id ? (
          <div>
            <dt className="inline font-medium text-nx-muted">Meeting ID: </dt>
            <dd className="inline">{booking.meeting_id}</dd>
          </div>
        ) : null}
        {booking.scheduled_for ? (
          <div>
            <dt className="inline font-medium text-nx-muted">Fecha/hora: </dt>
            <dd className="inline">{fmtDateTime(booking.scheduled_for)}</dd>
          </div>
        ) : null}
        {booking.google_calendar_event_id ? (
          <div>
            <dt className="inline font-medium text-nx-muted">Event ID: </dt>
            <dd className="inline break-all font-mono text-[11px]">
              {booking.google_calendar_event_id}
            </dd>
          </div>
        ) : (
          <div className="text-zinc-900">Sin event_id de Google Calendar</div>
        )}
        {booking.creation_method ? (
          <div>
            <dt className="inline font-medium text-nx-muted">Método: </dt>
            <dd className="inline">
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {tracking?.sequence_paused ? (
            <span className="inline-flex rounded-full bg-zinc-50 px-2 py-0.5 text-xs font-medium text-zinc-900 ring-1 ring-inset ring-zinc-600/20">
              Secuencia pausada
            </span>
          ) : null}
        </div>
      ...
      {responseLabel ? (
        <div className="rounded-lg border border-nx-border bg-nx-card-muted p-3 text-sm">
          <p className="text-xs text-nx-muted">Clasificación de la respuesta</p>
          <div className="mt-1 flex flex-wrap items-center gap-2">
            ...
            {classificationSummary ? (
              <span className="text-xs text-nx-muted">{classificationSummary}</span>
            ) : null}
            {lastSimulation?.reply_objective_label ||
            tracking?.last_reply_objective_label ? (
              <span className="inline-flex rounded-full bg-zinc-50 px-2 py-0.5 text-xs font-medium text-zinc-800 ring-1 ring-inset ring-zinc-600/20">
                Objetivo:{' '}
      ...
      {commercialDebug ? (
        <div className="rounded-lg border border-dashed border-zinc-300 bg-zinc-50/60 p-3 text-sm">
          <p className="text-xs font-semibold uppercase tracking-wide text-zinc-900">
            Estado comercial (simulación)
          </p>
          <dl className="mt-2 space-y-1 text-xs text-nx-ink">
            <div>
              <dt className="inline font-medium text-nx-muted">Texto: </dt>
              <dd className="inline">{commercialDebug.inbound_text}</dd>
            </div>
            <div>
              <dt className="inline font-medium text-nx-muted">Clasificación: </dt>
              <dd className="inline">
      ...
      {conversation.length ? (
        <div className="max-h-64 space-y-2 overflow-y-auto rounded-lg border border-nx-border bg-nx-card-muted p-3">
          {conversation.map((msg) => (
            <MessageBubble key={msg.id} msg={msg} />
          ))}
        </div>
      ) : (
        <p className="text-xs text-nx-muted">Todavía no hay mensajes en esta secuencia.</p>
      )}
